...
class WC_Checkout {
	...
	public function create_order( $data ) {
		...
		try {
			...
			//$order->set_total( 5555 /*WC()->cart->get_total( 'edit' )*/ );
	...
	public function create_order_line_items( &$order, $cart ) {
	    ...
	    //var_dump($cart->get_cart()); die;

        //$discount = (new Crm())->getCurrentUserDiscount();
		...
		foreach ( $cart->get_cart() as $cart_item_key => $values ) {
            ...
            $item->set_props(
				array(
					...
					/*'subtotal'     => $discount ? (new WooHelper())->getPersonalPrice($values['line_subtotal']) : $values['line_subtotal'],
					'total'        => $discount ? (new WooHelper())->getPersonalPrice($values['line_total']) : $values['line_total'],*/
                    'subtotal'     => $this->getPersonalPrice($values['line_subtotal'], $product),
                    //'total'        => ($this->getPersonalPrice($values['line_total'], $product) * $values['quantity']),
                    'total'        => $this->getPersonalPrice($values['line_total'], $product, $values['quantity']),
			...
			if ( $product ) {
				...
				/*var_dump($product->sale_price); die;
				if($product->is_type( 'variation' ) && $product->is_on_sale()) {
	...
	public function getPersonalPrice($price, $product = null, $qty = null)
    {
        $discount = (new Crm())->getCurrentUserDiscount();

        //echo $discount; die;

        if($product && $product->is_type( 'variation' ) && $product->is_on_sale()) {
            $price = $discount ? ((new WooHelper())->getPersonalSalePrice($product->regular_price, $product->sale_price) * $qty) : $price;
        } else {
            $price = $discount ? (new WooHelper())->getPersonalPrice($price) : $price;
        }

 ...
 * @version 2.1.0
 */
include_once $_SERVER['DOCUMENT_ROOT'].'/wp-content/themes/lion-of-porches/Crm.php';
include_once $_SERVER['DOCUMENT_ROOT'].'/wp-content/themes/lion-of-porches/WooHelper.php';
...
function wc_format_sale_price( $regular_price, $sale_price ) {
    ...
    $regular_price = str_replace(['&#8381;', ' ', ','], '', strip_tags($regular_price));
    $sale_price = str_replace(['&#8381;', ' ', ','], '', strip_tags($sale_price));

    // расчет персональной акционной цены
    $personal_sale_price = '';

    if ( is_user_logged_in() ) {
        //$personal_sale_price = (new WooHelper())->getVariationPriceTextSale($sale_price, $regular_price);
        $current_user = wp_get_current_user();
        $user_discount = (new Crm())->getUserDiscount($current_user->user_email);

        if($user_discount == 50) {
            // для уровня DL скидка 50% вне зависимости от скидки по акции
            $sale_discount = 50;
            $discount_price = $regular_price - ($regular_price * (50 / 100));
        } else {
            $sale_discount = (new WooHelper())->getSalePercent($sale_price, $regular_price);
            $sale_discount = (new Crm())->getUserSaleDiscount($sale_discount, $user_discount);
            $discount_price = $sale_price - ($sale_price * ($sale_discount / 100));
        }

        $data = sprintf('<span class="personal-data" data-personal-discount="%s" data-personal-price="%s" data-personal-economy="%s"></span>',
            //$sale_discount, ceil($discount_price), ceil($regular_price - $discount_price));
            $sale_discount, ceil($discount_price), ceil($regular_price - $discount_price));
    ...
    // расчет процента скидки для обычного клиента
    $sale_percent = (new WooHelper())->getSalePercent($sale_price, $regular_price);
//echo $sale_percent.'%';
    if($sale_percent) {
        $price = sprintf('<span class="sale-percent">-%s%%</span>', $sale_percent);
        $price .= $data;
        /*$price .= sprintf('<span class="personal-data" data-personal-discount="%s" data-personal-price="%s" data-personal-economy="%s"></span>',
            '', $personal_sale_price , '');*/
    }
